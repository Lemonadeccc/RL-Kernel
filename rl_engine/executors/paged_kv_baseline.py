...
class PagedKVScoringBaseline:
    ...
    wrapped model with `use_cache=True` when the model accepts that keyword. It
    does not implement a vLLM engine; it provides a local, reproducible memory
    reservation baseline for comparing stateless scoring against a generation
    path that keeps paged KV state alive while scoring a full sequence.
    """
